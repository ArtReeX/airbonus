...
function getCalculatedData(sql, name_db, session, callback) {
    'use strict';
    ...
    var data = {
        ...
        // выбор прямых рейсов
        selectDirectRoutes = function (done) {

            sql.request().query("SELECT airlines.name, routes.airline_id, routes.mile, routes.price, routes.source, routes.destination FROM " + name_db + ".routes,  " + name_db + ".airlines WHERE airlines.iata = routes.airline_id AND routes.source='" + session.userAirportFrom + "' AND routes.destination='" + session.userAirportTo + "' ORDER BY routes.mile", function (error, routes) {

                if (error) {
                    log.debug("Error SQL connection: " + error);
                    done();
                } else {
                    data.routes.direct = routes.recordset;
                    done();
                }
        ...
        selectBackRoutes = function (done) {

            sql.request().query("SELECT airlines.name, routes.airline_id, routes.mile, routes.price, routes.source, routes.destination FROM " + name_db + ".routes,  " + name_db + ". airlines WHERE airlines.iata = routes.airline_id AND routes.source='" + session.userAirportTo + "' AND routes.destination='" + session.userAirportFrom + "' ORDER BY routes.mile", function (error, routes) {

                if (error) {
                    log.debug("Error SQL connection: " + error);
                    done();
                } else {
                    data.routes.back = routes.recordset;
                    done();
                }
        ...
        selectAvailableCards = function (done) {
            ...
            async_module.series([
                ...
                function (done) {
                    ...
                    if (user_cards.length && data.authorized_airlines.length) {
                        ...
                        sql.request().query("SELECT cards.id, cards.name, cards.bonus_cur, cards.bonus_max, cards.amount, cards.fee1, cards.link, cards.image, cards.airline_iata, cards.id FROM " + name_db + ".cards WHERE cards.id IN (" + user_cards + ") AND cards.airline_iata IN (" + data.authorized_airlines + ") ORDER BY cards.amount", function (error, available_cards) {
                            ...
                            if (error) {
                                ...
                                data.cards.available = available_cards.recordset;
        ...
        calcCostAvailableCards = function (done) {
                ...
                function (done) {
                    ...
                    if (user_cards.length) {
                        ...
                        if (data.authorized_airlines.length) {
                            query = "SELECT cards.id, cards.name, cards.bonus_cur, cards.bonus_max, cards.amount, cards.fee1, cards.link, cards.image, cards.airline_iata FROM " + name_db + ".cards WHERE cards.id NOT IN (" + user_cards + ") AND cards.airline_iata IN (" + data.authorized_airlines + ") ORDER BY cards.fee1";
                        } else {
                            done();
                            return;
                        }
                        ...
                        if (data.authorized_airlines.length) {
                            query = "SELECT cards.id, cards.name, cards.bonus_cur, cards.bonus_max, cards.amount, cards.fee1, cards.link, cards.image, cards.airline_iata FROM " + name_db + ".cards WHERE cards.airline_iata IN (" + data.authorized_airlines + ") ORDER BY cards.fee1";
                        } else {
                            done();
                            return;
                        }
                    ...
                    sql.request().query(query, function (error, free_cards) {

                        if (error) {
                            log.debug("Error SQL connection: " + error);
                            done();
                        } else {
                            data.cards.free = free_cards.recordset;
                            done();
                        }
        ...
        calcCostFreeCards = function (done) {
        ...
        calcRecursive = function (combined_array, step, recursion_depth, temp_array, temp_array_params, criterion_calc, need_tickets, done) {
            ...
            if (step === recursion_depth || data.result.unsorted.length >= config_module.calculate.max_variants) {
                return;
            }
        ...
        sortResultAlgoritm = function (table_one, table_two) {
            ...
            return ((variant_two_tickets / variant_one_cards.length - (variant_two_fee1 / 100)) + (variant_two_have * 10)) - ((variant_one_tickets / variant_two_cards.length - (variant_one_fee1 / 100)) + (variant_one_have * 10));
        },
    ...
    async_module.parallel([
        ...
        function (done) {
            selectDirectRoutes(done);
        },

        // получение обратных рейсов
        function (done) {
            selectBackRoutes(done);
        }
            ...
            async_module.parallel([
                ...
                function (done) {
                    ...
                    async_module.series([
                        ...
                        function (done) {
                            selectAvailableCards(done);
                        },

                        function (done) {
                            calcCostAvailableCards(done);
                        }
                ...
                function (done) {
                    ...
                    async_module.series([
                        ...
                        function (done) {
                            selectFreeCards(done);
                        },

                        function (done) {
                            calcCostFreeCards(done);
                        }
