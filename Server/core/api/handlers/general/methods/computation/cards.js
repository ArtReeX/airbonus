...
module.exports.selectConversion = (
    ...
    let cards_use_count,
        ...
        checkArrayToUnique = array_cards => {
            ...
            for (
                array_count_one = 0;
                array_count_one < array_cards.length;
                array_count_one += 1
            ) {
                for (
                    array_count_two = array_count_one + 1;
                    array_count_two < array_cards.length;
                    array_count_two += 1
                ) {
                    if (
                        Number(array_cards[array_count_one].params.card_id) ===
                        Number(array_cards[array_count_two].params.card_id)
                    ) {
                        return false;
                    }
                }
        ...
        sortAlhoritmCards = (card_one, card_two) => {
            ...
            if (Boolean(card_one.card.have) !== Boolean(card_two.card.have)) {
                ...
                if (Boolean(card_two.card.have)) {
                    return 1;
                }
            }

            if (
                Number(card_one.params.bonus_cur) !== Number(card_two.bonus_cur)
            ) {
                if (
        ...
        generateUniqueId = (current_card, array_with_card) => {
            ...
            for (
                ...
                current_card_count < array_with_card.length;
                current_card_count += 1
            ) {
                array_with_card_id.push(
                    Number(array_with_card[current_card_count].params.card_id)
                );
            }
            ...
            for (
                current_card_count = 0;
                current_card_count < array_with_card_id.length;
                current_card_count += 1
            ) {
                common_identifier += String(
                    array_with_card_id[current_card_count]
                );
            }

            // возврат уникального идентификатора
            return Number(
                String(current_card.params.card_id) + String(common_identifier)
            );
        },
        ...
        checkCardsToUnique = (current_card, array_with_card) => {
            ...
            for (
                array_with_card_count = 0;
                array_with_card_count < cards_conversion.length;
                array_with_card_count += 1
            ) {
                // перебор карт, используемые для конвертации внутри карт
                for (
                    converted_card_count = 0;
                    converted_card_count <
                    cards_conversion[array_with_card_count].converted_cards
                        .length;
                    converted_card_count += 1
                ) {
                    array_all_card_id.push(
                        Number(
                            cards_conversion[array_with_card_count]
                                .converted_cards[converted_card_count].params
                                .card_id
                        )
                    );
                }
                ...
                if (
                    String(
                        String(current_card.card.id) +
                            JSON.stringify(array_with_card_id.sort())
                    ) ===
                    String(
                        String(
                            cards_conversion[array_with_card_count].card.id
                        ) + JSON.stringify(array_all_card_id)
                    )
                ) {
                    return false;
                }
        ...
        calcRecursive = (
            ...
            let array_count,
                ...
                array_program_current_count,
                conversion_factors_count,
                current_card_program_id,
                array_card_program_id,
                factor = 1;

            for (
                array_count = 0;
                array_count < start_conversion_cards.length;
                array_count += 1
            ) {
                ...
                array_card_program_id = start_conversion_cards[
                    array_count
                ].card.program_id.split(",");

                // поиск курса обмена
                for (
                    conversion_program_current_count = 0;
                    conversion_program_current_count <
                    current_card_program_id.length;
                    conversion_program_current_count += 1
                ) {
                    for (
                        array_program_current_count = 0;
                        array_program_current_count <
                        array_card_program_id.length;
                        array_program_current_count += 1
                    ) {
                        for (
                            conversion_factors_count = 0;
                            conversion_factors_count <
                            conversion_factors.length;
                            conversion_factors_count += 1
                        ) {
                            if (
                                Number(
                                    current_card_program_id[
                                    ...
                                    conversion_factors[conversion_factors_count]
                                        .to_program_id &&
                                Number(
                                    array_card_program_id[
                                        array_program_current_count
                ...
                temp_array_params.sum_amount += Number(
                    start_conversion_cards[array_count].params.amount
                );
                temp_array_params.sum_fee1 += Number(
                    start_conversion_cards[array_count].params.fee1
                );
                temp_array_params.sum_bonus_cur += Number(
                    start_conversion_cards[array_count].params.bonus_cur *
                        factor
                );
                ...
                if (
                    checkArrayToUnique(temp_array) &&
                    start_conversion_cards[array_count].params.card_id !==
                        current_card.params.card_id &&
                    checkCardsToUnique(current_card, temp_array)
                ) {
                    // добавление карты
                    cards_conversion.push({
                        ...
                        params: {
                            card_id: Number(
                                generateUniqueId(
                                    cards_use_in_computation[cards_use_count],
                                    temp_array
                                )
                            ),
                            converted_from_card: Number(
                                cards_use_in_computation[cards_use_count].params
                                    .converted_from_card
                            ),
    ...
    async.series(
        [
                            ...
                            conn.query(
                                ...
                                (error, cards) => {
                                    if (error) {
                                        ...
                                        for (
                                            cards_db_count = 0;
                                            cards_db_count < cards.length;
                                            cards_db_count += 1
                                        ) {
                                            if (
                                                amex_cards.indexOf(
                                                    Number(
                                                        cards[cards_db_count].id
                                                    )
                                                ) === -1
                                            ) {
                                                cards_all.push({
                                        ...
                                        for (
                                            cards_db_count = 0;
                                            cards_db_count < cards_all.length;
                                            cards_db_count += 1
                                        ) {
                                            cards_all[
                                                cards_db_count
            ...
            callback => {
                ...
                for (
                    cards_use_count = 0;
                    cards_use_count < user_cards.length;
                    cards_use_count += 1
                ) {
                    for (
                        cards_all_count = 0;
                        cards_all_count < cards_all.length;
                        cards_all_count += 1
                    ) {
                        // проверка на совпадение идентификатора
                        if (
                            Number(user_cards[cards_use_count].card) ===
                            Number(cards_all[cards_all_count].card.id)
                        ) {
                            // замена данных карты
                            cards_all[
                                cards_all_count
                            ].card.bonus_cur = cards_all[
                                cards_all_count
                            ].params.bonus_cur = Number(
                                user_cards[cards_use_count].bonus
                            );
                            cards_all[cards_all_count].card.amount = cards_all[
                                cards_all_count
                            ].params.amount = 0;
            ...
            callback => {
                for (
                    ...
                    end_conversion_id = cards_use_in_computation[
                        cards_use_count
                    ].card.program_id.split(",");

                    for (
                        programs_count = 0;
                        programs_count < end_conversion_id.length;
                        programs_count += 1
                    ) {
                        // выбор всех идентификаторов карт, которые могут преобразовываться в нашу карту
                        for (
                            conversion_count = 0;
                            conversion_count < conversion_factors.length;
                            conversion_count += 1
                        ) {
                            // добавление записи о возможности использования карты для преобразования
                            if (
                                Number(
                                    conversion_factors[conversion_count]
                                        .to_program_id
                                ) === Number(end_conversion_id[programs_count])
                            ) {
                                start_conversion_id.push(
                                    Number(
                                        conversion_factors[conversion_count]
                                            .from_program_id
                                    )
                    ...
                    for (
                        start_conversion_id_count = 0;
                        start_conversion_id_count < start_conversion_id.length;
                        start_conversion_id_count += 1
                    ) {
                        for (
                            start_conversion_cards_count = 0;
                            start_conversion_cards_count < cards_all.length;
                            start_conversion_cards_count += 1
                        ) {
                            // выбор всех идентификаторов карты, которые она содержит
                            start_conversion_id_one_card = cards_all[
                                start_conversion_cards_count
                            ].card.program_id.split(",");

                            for (
                                start_conversion_id_one_card_count = 0;
                                start_conversion_id_one_card_count <
                                start_conversion_id_one_card.length;
                                start_conversion_id_one_card_count += 1
                            ) {
                                ...
                                if (
                                    ...
                                    cards_use_in_computation[cards_use_count]
                                        .params.card_id !==
                                        cards_all[start_conversion_cards_count]
                                            .params.card_id
                                ) {
                                    // добавление карты
                                    start_conversion_cards.push(
                                        cards_all[start_conversion_cards_count]
                                    );
                                }
                            }
                    ...
                    calcRecursive(
                        ...
                        conversion_factors,
                        cards_use_in_computation[cards_use_count],
                        start_conversion_cards.sort(sortAlhoritmCards)
                    );
                    ...
                    start_conversion_cards.splice(
                        0,
                        start_conversion_cards.length
                    );
                }
...
module.exports.calcCostConversionCards = (data, params, callback) => {
    ...
    for (
        card_count = 0;
        card_count < data.cards.conversion.length;
        card_count += 1
    ) {
        //--------------------- прямой маршрут ---------------------//
        ...
        for (
            ...
            for (
                ...
                if (
                    data.cards.conversion[card_count].params.bonus_cur >=
                        Number(
                            data.routes.direct[route_count].price_miles
                                ...
                                : Number(data.routes.direct[route_count].miles)
                        ) *
                            Number(people_count) &&
                    data.cards.conversion[card_count].card.airline_iata ===
                        data.routes.direct[route_count].airline_iata &&
                    data.cards.conversion[card_count].params.amount +
                        data.cards.conversion[card_count].params.fee1 <=
                        Number(
                    ...
                    data.routes_cost.conversion.direct.push({
                        card: String(
                            data.cards.conversion[card_count].card.name
                        ),

                        card_id: Number(
                            data.cards.conversion[card_count].params.card_id
                        ),
                        ...
                        fee1: Number(
                            data.cards.conversion[card_count].card.fee1
                        ),

                        amount: Number(
                            data.cards.conversion[card_count].card.amount
                        ),
                        ...
                        mile:
                            Number(
                                data.routes.direct[route_count].price_miles
                                    ...
                                    : Number(
                                          data.routes.direct[route_count].miles
                                      )
                            ) * people_count,

                        available_amount_of_bonuses: Number(
                            data.cards.conversion[card_count].card.bonus_cur
                        ),
                        ...
                        link: String(
                            data.cards.conversion[card_count].card.link
                        ),

                        image: String(
                            data.cards.conversion[card_count].card.image
                        ),

                        have: Boolean(
                            data.cards.conversion[card_count].card.have
                        ),
                        ...
                        converted_cards:
                            data.cards.conversion[card_count].converted_cards
                    });
                }
            }
        }
                    ...
                    data.cards.conversion[card_count].params.bonus_cur >=
                        Number(
                            data.routes.back[route_count].price_miles
                                ...
                                : Number(data.routes.back[route_count].miles)
                        ) *
                            Number(people_count) &&
                    data.cards.conversion[card_count].card.airline_iata ===
                        ...
                        mile:
                            Number(
                                data.routes.back[route_count].price_miles
                                    ...
                                    : Number(
                                          data.routes.back[route_count].miles
                                      )
                            ) * people_count,
