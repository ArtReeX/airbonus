...
$(document).ready(() => {
    ...
    window.socket.off("computation_get").on("computation_get", result => {
        // проверка соответствие обработчика со страницей
        if (window.identifier === "computation") {
            // сохранение результатов вычисления в памяти
            window.data.computation_results = JSON.parse(
                JSON.stringify(result.data)
            );
            window.data.modified_results = JSON.parse(
                JSON.stringify(result.data)
            );
            ...
            if (result.data.computation.length) {
                ...
                $("#computation-statistics-combination")
                    .text(String(result.data.treated_combinations))
                    .spincrement({ duration: 800 });
                $("#computation-statistics-cards")
                    .text(String(result.data.number_of_cards))
                    .spincrement({ duration: 800 });
                ...
                // обработка данных
                result.data.computation.forEach(
                    (computation, computation_count) => {
                        // добавление заголовка со счётчиком варианта
                        $("#computation-result_tables").append(
                        ...
                        // определение блоков
                        $("#computation-result_tables").append(
                            "<div id='computation-result_tables_variant_" +
                                computation_count +
                                "' variant='" +
                                computation_count +
                                "'></div>"
                        ...
                        // прямые цепочки
                        $(
                            "#computation-result_tables_variant_" +
                                computation_count
                        ).append(
                            "<div class='row p-3' id='direct_variants'></div>"
                        );
                        ...
                        $(
                            "#computation-result_tables_variant_" +
                                computation_count
                        ).append("<div class='col-12' id='direct_info'></div>");
                        ...
                        $(
                            ...
                            "<div class='row p-3' id='back_variants'></div>"
                        );
                        ...
                        $(
                            "#computation-result_tables_variant_" +
                                computation_count
                        ).append("<div class='col-12' id='back_info'></div>");

                        // добавление карт для прямого перелёта в таблицу вариантов
                        computation.direct.variants.forEach(variant => {
                            $(
                                "#computation-result_tables_variant_" +
                                    computation_count +
                                    " #direct_variants"
                                ...
                                "<div class='col-12 col-lg-6' card='" +
                                    String(variant.card_id) +
                                    "'>" +
                                    "<div class='card mb-3 bg-content " +
                                    String(
                                        variant.have
                                            ? "border-success"
                                            : "border-primary"
                                    ...
                                    String(
                                        variant.conversion
                                            ? "<br> <span class='badge badge-info'>card to accept transferred miles</span>"
                                            : ""
                                    ) +
                                    "</div>" +
                                    ...
                                    "<div class='col-6 ml-auto'>" +
                                    String(
                                        variant.available_amount_of_bonuses
                                    ) +
                                    "</div>" +
                                    "</div>" +
                                    "</div>" +
                            ...
                            if (variant.converted_cards) {
                                variant.converted_cards.forEach(
                                    conversion_variant => {
                                        $(
                                            "#computation-result_tables_variant_" +
                                                computation_count +
                                            ...
                                            "<div class='col-12 col-lg-6' card='" +
                                                String(
                                                    conversion_variant.params
                                                        .card_id
                                                ) +
                                                "'>" +
                                                "<div class='card mb-3 bg-content " +
                                                String(
                                                    conversion_variant.card.have
                                                        ? "border-success"
                                                        : "border-primary"
                                                ) +
                                                " text-white'>" +
                        ...
                        computation.back.variants.forEach(variant => {
                            $(
                                "#computation-result_tables_variant_" +
                                    computation_count +
                                    " #back_variants"
