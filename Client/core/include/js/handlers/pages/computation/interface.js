...
$(document).ready(function () {
    ...
    $("#loaded").off("click", "#computation-button_set_price").on("click", "#computation-button_set_price", function () {
        ...
        var element = this,
            ...
            numberOfTimesTheCardIsUsed = function (current_card, array) {
        ...
        window.data.modified_results.computation[Number(variant)].direct.variants.forEach(function (card_computation, count) {
            ...
            if (String(card_computation.airline) === String(airline)) {
                ...
                // вычисление данных
                card_computation.tickets = Math.floor(Number(Number(card_computation.params.bonus_cur) / Number(Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants)) ? price_of_one_ticket : 0) + Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants)) ? price_of_one_ticket : 0))) * Number(Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants)) ? numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants) : 1) / Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants)) ? numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants) : 1)));
        ...
        window.data.modified_results.computation[Number(variant)].back.variants.forEach(function (card_computation, count) {
            ...
            if (String(card_computation.airline) === String(airline)) {
                ...
                // вычисление данных
                card_computation.tickets = Math.floor(Number(Number(card_computation.params.bonus_cur) / Number(Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants)) ? price_of_one_ticket : 0) + Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants)) ? price_of_one_ticket : 0))) * Number(Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants)) ? numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].back.variants) : 1) / Number(Boolean(numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants)) ? numberOfTimesTheCardIsUsed(card_computation, window.data.computation_results.computation[Number(variant)].direct.variants) : 1)));
                card_computation.price_of_one_ticket = Number(price_of_one_ticket);
