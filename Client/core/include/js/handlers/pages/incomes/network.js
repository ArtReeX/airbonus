...
$(document).ready(() => {
    ...
    window.socket.off("incomes_get").on("incomes_get", result => {
    ...
    window.socket.off("incomes_set").on("incomes_set", result => {
        // проверка соответствие обработчика со страницей
        if (window.identifier === "incomes") {
            if (result.error) {
                // ОБРАБОТКА ОШИБОК
                ...
                if (result.error.type === "database") {
                    ...
                    $("#incomes-info_warning")
                        .text(
                            "We think you income or credit score is too low to be approved for most of the credit cards."
                        )
                        .show();
