...
$(document).ready(() => {
    ...
    window.socket.off("cards_get_all").on("cards_get_all", result => {
    ...
    window.socket.off("cards_set_all").on("cards_set_all", result => {
        // проверка соответствие обработчика со страницей
        if (window.identifier === "cardsAll") {
            if (result.error) {
                // ОБРАБОТКА ОШИБОК
