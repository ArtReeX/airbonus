...
$(document).ready(() => {
    ...
    $("#loaded")
        .off("keyup", "#airports-from")
        .on("keyup", "#airports-from", () => {
    ...
    $("#loaded")
        .off("keyup", "#airports-to")
        .on("keyup", "#airports-to", () => {
            // проверка соответствие обработчика со страницей
            if (window.identifier === "airports") {
                // скрытие другого списка с подсказками
                $("#airports-from_tips")
    ...
    $("#loaded")
        .off("click", "#airports-from_tips .list-group-item")
        .on("click", "#airports-from_tips .list-group-item", event => {
            // проверка соответствие обработчика со страницей
            if (window.identifier === "airports") {
    ...
    $("#loaded")
        .off("click", "#airports-to_tips .list-group-item")
        .on("click", "#airports-to_tips .list-group-item", event => {
            // проверка соответствие обработчика со страницей
            if (window.identifier === "airports") {
                // перенос значения из списка подсказок в поле ввода
                $("#airports-to").val(
    ...
    $("#loaded")
        .off("click", "#airports-button_goto_next")
        .on("click", "#airports-button_goto_next", () => {
            // проверка соответствие обработчика со страницей
            if (window.identifier === "airports") {
                // скрытие списков с подсказками
                $("#airports-from_tips, #airports-to_tips")
                    .empty()
                    .hide();
